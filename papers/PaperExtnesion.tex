...
When evaluating CIC3 processes, we want to capture a few aspects of the system. The first is global attainment: we want to know how much of each contagion's quota was filled. The following definition matches this goal by only including infected nodes towards the numerator up to a given contagion's quota. Let $\mathcal{C}$ be the set of contagions and let $C_i \in \mathcal{C}$ denote contagion $i$. Let $I_i^{\text{raw}}$ be the set of nodes that were infected by $C_i$, let $t(v)$ denote the infection timestep of node $v \in I_i^{\text{raw}}$, and let $Q_i$ be the quota for $C_i$. Define the capped infection count
\begin{equation}
K_i = \min\!\big(|I_i^{\text{raw}}|,\, Q_i\big).
\end{equation}
Then per-contagion attainment and global attainment are
\begin{equation}
\text{attainment}_i = \frac{K_i}{Q_i} \in [0,1],
\end{equation}
\begin{equation}
\text{global attainment} = \frac{1}{|\mathcal{C}|} \sum_{C_i \in \mathcal{C}} \text{attainment}_i \in [0,1].
\end{equation}
...
\begin{equation}
K_i^{\text{td}} = \min\!\left( \sum_{k=1}^{\min(Q_i,\,|I_i^{\text{raw}}|)} V\!\big(t_{i,(k)}\big),\; Q_i \right).
\end{equation}
Then
\begin{equation}
\text{attainment}_i^{\text{td}} = \frac{K_i^{\text{td}}}{Q_i} \in [0,1],
\end{equation}
\begin{equation}
\text{global attainment}^{\text{td}} = \frac{1}{|\mathcal{C}|} \sum_{C_i \in \mathcal{C}} \text{attainment}_i^{\text{td}} \in [0,1].
\end{equation}

Prior metrics for network contagion do not capture both a hard quota and a time-discounted value under that quota. The following table summarizes why.
...
\bottomrule
\end{tabular}
\caption{Prior contagion metrics lack a hard quota with zero marginal value and lack a quota-aware time discount, both required for CIC3 evaluation.}
